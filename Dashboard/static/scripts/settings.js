...
const SCHEMA = [
  ...
  },
  {
    ...
    fields: [
      ...
      { path: "browser_detection", label: "Personendetectie in browser (alleen preview)", type: "bool",
        hint: "Laadt ~4 MB TensorFlow.js en belast de CPU. Toont alleen een kader om gedetecteerde personen op dit scherm — stuurt geen lamp aan (dat doet uitsluitend de server, zie Aanwezigheidsdetectie). Standaard uit." },
      ...
      { path: "lamp_quiet_from", label: "Lamp-stil vanaf (niet meer in gebruik)", type: "text",
        hint: "Oud, ongebruikt sinds de lampbesturing volledig server-side gebeurt. Gebruik in plaats daarvan Aanwezigheidsdetectie -> auto_light_block_after." },
      { path: "lamp_quiet_to", label: "Lamp-stil tot (niet meer in gebruik)", type: "text" },
    ],
  },
  {
    ...
    fields: [
      ...
      { path: "auto_light_enabled", label: "Automatische lamp op aanwezigheid", type: "bool",
        hint: "Zet je deze lamp handmatig via Devices, dan laat de automatisering 'm met rust tot de kamer weer wisselt van bezet naar leeg (of andersom) — zie mode in /api/presence." },
      { path: "lamp", label: "Welke lamp (index of naam)", type: "text",
        hint: "Zelfde lamp als je hierboven kiest bij Devices — scenes (feest/bureau/normaal) blijven gewoon apart handmatig bedienbaar." },
